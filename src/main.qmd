...
df_mean_tc <- df_raw %>%
  pivot_longer(
    cols = c(matches("TC._treatment"), matches("TC._seconds_user_done$")),
    names_to = c("tc", ".value"), names_pattern = "TC(.)_(.*)"
  ) %>%
  select("ID", "tc", "treatment", "seconds_user_done")

df_mean_tc <- df_mean_tc %>%
  group_by(tc, treatment) %>%
  summarise(mean_seconds=round(mean(seconds_user_done), 0), .groups = "drop")
